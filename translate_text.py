...
def traducir_con_openai(texto, client):
...
El resultado debe sonar como si fuera escrito originalmente en español."""
        ...
        tokens_entrada = len(texto) // 4  # Aproximación
        tokens_salida = len(traduccion) // 4
        costo = (tokens_entrada * 0.00015 + tokens_salida * 0.0006) / 1000  # GPT-4o-mini pricing
...
def traducir_con_google(texto):
    ...
    try:
        ...
        max_chunk = 4500  # Google tiene límite de ~5000 chars
...
def traducir_texto(texto):
    ...
    openai_disponible, client = verificar_openai()
    
    if openai_disponible:
        try:
            return traducir_con_openai(texto, client)
        except Exception as e:
            print(f"⚠️  OpenAI falló: {e}")
            print("🔄 Cambiando a Google Translate...")
    
    # Intentar Google Translate como respaldo
...
def traducir_automatico(ruta_proyecto):
    ...
    try:
        # Buscar transcripción en el proyecto
...
def main():
    ...
    # Buscar proyectos con transcripción
